...
<title>NotTrello - ${pergunta.titulo}</title>
</head>
			...
			<div class="col-md-8">
				...
				<ul class="list-group">
					<li
						...
						style="text-decoration: none;">
							...
							</div> <small><img class="avatar" src="/imgs/padrao.jpg"
								style="width: 30px;"> ${pergunta.usuario.nomeUsuario}</small>
				...
				<c:choose>
					<c:when test="${usuarioLogado != null }">
						<a href="#" data-toggle="modal" data-pergunta="${pergunta.id}"
							data-id="${usuarioLogado.id}" data-target="#myModalResposta">Responder</a>
					</c:when>
					<c:otherwise>
				...
				<ul class="list-group">
					<c:forEach var="resposta" items="${respostas}">
						<li class="list-group-item list-group-item-light">
							<div class="d-flex w-100 justify-content-between">
								<small> <img class="avatar" src="/imgs/padrao.jpg"
									style="width: 30px;"> ${resposta.usuario.nomeUsuario}

								</small>
								<p>${resposta.data_criacao}</p>

							</div>
							<p>${resposta.conteudo}</p> <a
							href="/forum/curtir/resposta?id=${resposta.id}"><i
								class="fas fa-thumbs-up"></i> ${resposta.curtir}</a> <a
							href="/forum/discurtir/resposta?id=${resposta.id}"><i
								class="fas fa-thumbs-down"></i> ${resposta.discurtir}</a>
						</li>
					</c:forEach>
				</ul>
